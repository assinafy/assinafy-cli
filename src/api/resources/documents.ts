...
export class DocumentResource extends BaseResource {
	...
	/**
	 * Rename a document (`PATCH /documents/{id}`).
	 *
	 * Only permitted before the signature process starts — the API returns a
	 * 400 once an assignment is active.
	 */
	async rename(documentId: string, name: string): Promise<IDocumentDetailsResponse> {
		...
		return this.call('Failed to rename document', () =>
			this.http.patch(`/documents/${id}`, { name }),
		);
	}
	...
	async waitUntilReady(
		...
		while (Date.now() - start < maxWaitMs) {
			...
			try {
				...
				// A client-side error (bad/unknown id, unauthorized, forbidden) will
				// never become "ready" — surface it immediately instead of masking it
				// as a timeout after the full maxWaitMs. Keep retrying transient
				// failures (network blips, 5xx).
